#!/bin/bash
# 
# File: run_rsv_alignment_pipeline_RSVB.sh by Marc Perry
# 
# Last Updated: 2024-06-17; Status: testing in production
...
do
  cd "${DIR}";
  ../rsv_alignment_pipeline_RSVB.sh "${FQ1}" "${FQ2}" >& logfile.txt;
  ...
  done < ../RSVB.non-redundant-inserts.bed

  head -n 1 actual_untrimmed_nonredundant_covg_per_amplicon.tsv > "${DIR}"_actual_untrimmed_coverage_per_amplicon.tsv
  grep ^RE actual_untrimmed_nonredundant_covg_per_amplicon.tsv >> "${DIR}"_actual_untrimmed_coverage_per_amplicon.tsv
